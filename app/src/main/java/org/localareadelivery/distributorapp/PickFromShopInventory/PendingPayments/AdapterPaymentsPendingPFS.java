...
import org.localareadelivery.distributorapp.ModelPickFromShop.OrderStatsPFS;
import org.localareadelivery.distributorapp.OrderHistoryPFS.Utility.UtilityOrderStatusPFS;
 ...
 */
class AdapterPaymentsPendingPFS extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    AdapterPaymentsPendingPFS(List<OrderPFS> dataset, NotifyConfirmOrder notifyConfirmOrder, Fragment fragment) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if(viewType==VIEW_TYPE_ORDER)
        {
            view = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.list_item_order_payment_pending_pfs,parent,false);
...
//        return -1;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holderVH, int position) {
        ...
        else if(holderVH instanceof LoadingViewHolder)
        {
            ...
            if(fragment instanceof PaymentsPendingFragmentPFS)
            {
                int items_count = ((PaymentsPendingFragmentPFS) fragment).item_count;
    ...
    class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        @Bind(R.id.button) TextView setPackedButton;
...
//        TextView confirmOrderButton;
        ...
        {
            notifyConfirmOrder.notifySetOrderPacked(dataset.get(getLayoutPosition()),getLayoutPosition());
        }
    ...
    interface NotifyConfirmOrder{
        void notifySetOrderPacked(OrderPFS order, int position);
        void notifyOrderSelected(OrderPFS order);
        void notifyCancelOrder(OrderPFS order, int position);
    }
