 ...
 */
class AdapterOrdersPendingPFS extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    AdapterOrdersPendingPFS(List<OrderPFS> dataset, NotifyConfirmOrder notifyConfirmOrder, Fragment fragment) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if(viewType==VIEW_TYPE_ORDER)
        {
            view = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.list_item_order_pending_pfs,parent,false);
...
//        return -1;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holderVH, int position) {
        ...
        else if(holderVH instanceof LoadingViewHolder)
        {
            ...
            if(fragment instanceof PendingOrdersFragmentPFS)
            {
                int items_count = ((PendingOrdersFragmentPFS) fragment).item_count;
